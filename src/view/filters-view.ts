...
class FiltersView {
  ...
  private static fillRange(
    ...
    FiltersView.setRangeHandler(rangeInputs[0], true, rangeInputs[0], rangeInputs[1], minGap, rangeFrom, isPrice, rangeBar, min, max);
    FiltersView.setRangeHandler(rangeInputs[1], false, rangeInputs[0], rangeInputs[1], minGap, rangeTo, isPrice, rangeBar, min, max);
  }
  ...
  private static setRangeHandler(
    ...
    range.addEventListener('input', () => {
      ...
      if (rightValue - leftValue <= minGap) {
        range.value = (isLeft ? (rightValue - minGap) : (leftValue + minGap)).toString();
      }
