...
import addOrderValidation from '../order-validation';
...
class CartView {
  ...
  private static fillProductsInfo(
    ...
    cartProducts.forEach((item, index) => {
      if (
        index >= CartView.pageLimit * (CartView.currentPage - 1) &&
        index < CartView.pageLimit * CartView.currentPage
      ) {
  ...
  private static setPromoInputHandler(parentBlock: HTMLElement): void {
    ...
    if (input instanceof HTMLInputElement) {
      input.addEventListener('input', () => {
        const promoInfo = (input.parentElement as HTMLElement).querySelector(
          '.summary__promo-info',
        ) as HTMLElement;
        if (input.value.length === 0) {
          ...
          if (
            ...
            const addButton = promoInfo.querySelector(
              '.summary__add',
            ) as HTMLElement;
  ...
  private static setPromoButtonsHandler(parentBlock: HTMLElement): void {
    ...
    summary.addEventListener('click', (event: Event) => {
      if (
        ...
        !event.target.classList.contains('summary__button')
      ) {
        const code = event.target.dataset.promo as string;
        if (event.target.classList.contains('summary__add'))
          CartView.setAddPromoHandler(event.target, code);
        if (event.target.classList.contains('summary__drop'))
          CartView.setDropPromoHandler(event.target, code);
      }
    });
  }

  private static setAddPromoHandler(
  ...
  private static setDropPromoHandler(
    button: HTMLButtonElement,
    code: string,
  ): void {
  ...
  private static renderAppliedCodes(): Element | null {
    const appliedCodes: Element | null = document.querySelector(
      '.summary__applied-codes',
    );
    let codesList: HTMLElement | null = null;
    if (appliedCodes instanceof HTMLElement) {
      codesList = appliedCodes.querySelector(
        '.summary__codes-list',
      ) as HTMLElement;
      codesList.innerHTML = '';
      if (CartView.isPromoApplied) {
        ...
        Object.entries(CartView.promocodes).forEach((item) => {
          if (item[1].isApplied) {
            ...
            Builder.fillTextBlock(
              appliedCode,
              'span',
              `${item[1].text} - ${item[1].percent}%`,
            );
            const dropButton = appliedCode.querySelector(
              '.summary__drop',
            ) as HTMLElement;
            const code = item[0];
  ...
  private static handlePromoTotalSum(): void {
    const totalPrice: Element | null = document.querySelector(
      '.summary__total',
    );
    let promoTotalPrice: Element | null = document.querySelector(
      '.summary__total--promo',
    );
      ...
      if (promoTotalPrice instanceof Element) {
        if (!CartView.isPromoApplied) {
          ...
          Builder.fillTextBlock(
            promoTotalPrice,
            'span',
            `$${CartView.calculatePromoSum()}`,
          );
        }
      } else {
        promoTotalPrice = totalPrice.cloneNode(true) as Element;
        totalPrice.parentElement?.insertBefore(
          promoTotalPrice,
          totalPrice.nextElementSibling,
        );
  ...
  private static calculatePromoSum(): string {
    const promoPrecent: number = Object.values(CartView.promocodes).reduce(
      (sum, item) => {
        let newSum = sum;
  ...
  private static setPaginationBtnsHandler(
    ...
    const prevBtn = cartPage.querySelector(
      '.pagination__button--prev',
    ) as HTMLElement;
    const nextBtn = cartPage.querySelector(
      '.pagination__button--next',
    ) as HTMLElement;
    CartView.currentPageBlock = cartPage.querySelector(
      '.pagination_number',
    ) as HTMLElement;
  ...
  private static setPaginationInputHandler(
    ...
    const limitInput = cartPage.querySelector(
      '.pagination__limit-input',
    ) as HTMLInputElement;
